...
def pdf_tools_gui():
    ...
    Button(frame, text="Splice PDFs (coming soon)", command=split_pdfs).grid(column=0, row=2)
    Button(frame, text="Extract text (coming soon)", command=extract_text).grid(column=0, row=3)
    Button(frame, text="Extract images (coming soon)", command=extract_images).grid(column=0, row=4)
